...
class InPlantRembang extends CI_Controller {
	function __construct() {
        parent::__construct();
        if (!$this->session->userdata('is_login'))
            redirect('login');
        $this->load->model('InPlantRembang_model');
    }

    function index() {
        $data = array('title' => 'Semen Gresik');
        $this->template->display('InPlantRembang_view', $data);
    }

    function detailTruk() {
        $data = array('title' => 'Detail Truk', 'url' => 'sg/InPlantRembang/dataDetailTruk/', 'nm_plant' => 'Rembang');
    ...
    function dataDetailTruk() {
        ...
        switch ($id) {
            case 0 :
                $data = $this->InPlantRembang_model->detailBagCargo();
                break;
            case 1 :
                $data = $this->InPlantRembang_model->detailBulkCargo();
                break;
            case 2 :
                $data = $this->InPlantRembang_model->detailBagTmbgn();
                break;
            case 3 :
                $data = $this->InPlantRembang_model->detailBulkTmbgn();
                break;
        }
    ...
    function detailConveyor() {
        $data = array('title' => 'Detail Truk', 'url' => "sg/InPlantRembang/dataDetailConveyor/");
    ...
    function dataDetailConveyor() {
        ...
        $data_fix = array();
        $arr = array();
        $data = $this->InPlantRembang_model->detailConveyor($id);
        if (!empty($data)) {
            foreach ($data as $d) {
                if (empty($arr)) {
    ...
    function detailTonase() {
        $data = array('title' => 'Detail Tonase', 'url' => "sg/InPlantRembang/dataDetailTonase/", 'nm_plant' => 'rembang');
    ...
    function dataDetailTonase() {
        $id = $this->uri->segment(4);
        if ($id == 'bag') {
            $data = $this->InPlantRembang_model->detailBagTonase();
        } else if ($id == 'bulk') {
            $data = $this->InPlantRembang_model->detailBulkTonase();
        }
        $subTotalPOZak = 0;
        $subTotalPOTon = 0;
        $subTotalSalesZak = 0;
    ...
    function avgcargo() {
    	$data = array();
        $data = $this->InPlantRembang_model->avgcargo();

        return $data;
    }

    function avgtmbgn() {
        $data = $this->InPlantRembang_model->avgtmbgn();
        //echo json_encode($data);
        return $data;
    }

    function cntcargo() {
        $data = $this->InPlantRembang_model->cntcargo();
        //echo json_encode($data);
        return $data;
    }

    function cnttmbgn() {
        $data = $this->InPlantRembang_model->cnttmbgn();
        //echo json_encode($data);
        return $data;
    }

    function avgpbrk() {
        $data = $this->InPlantRembang_model->avgpbrk();
        //echo json_encode($data);
        return $data;
    }

    function cntPbrk(){
    	$cntPbrk = $this->InPlantRembang_model->get_data_conveyor();
    	$data = array();
    	if (!empty($cntPbrk)) {
    ...
    function waktuUpdate() {
        $data = $this->InPlantRembang_model->waktuUpdate();
        //echo json_encode($data);
        return $data;
    }

    function overall() {
        $aarFromStok = $this->InPlantRembang_model->sql_stok();
        $aarInplant = $this->InPlantRembang_model->tonaseOverall();
        $results = array();
        $results['BAG']['DIST'] = 0;
        $results['BAG']['DA'] = 0;
        $results['BAG']['SG'] = 0;
    ...
    function avgOverall() {
        $data = $this->InPlantRembang_model->avgOverall();
        //echo json_encode($data);
        return $data;
    }
    
    function sisaSO(){
        $data = $this->InPlantRembang_model->sisaSO();
        return $data;
    }
