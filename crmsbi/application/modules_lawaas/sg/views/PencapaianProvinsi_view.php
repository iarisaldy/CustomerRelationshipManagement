...
//                "<th class='info' width='10%'>MARKETSHARE HARIAN</th>" +
        ...
        if (isRealValue(obj_data)) {
            $.each(obj_data.sales, function (key, val) {
                ...
                //pewarnaan ms harian
                if (Math.round(val.RKAP_MS.toString().replace(',', '.') - val.MARKETSHARE.toString().replace(',', '.')) >= 2) {
                    var warnamsHarian = "class='label label-merah'";
                } else if (Math.round(val.RKAP_MS.toString().replace(',', '.') - val.MARKETSHARE.toString().replace(',', '.')) < 2 && Math.round(val.RKAP_MS.toString().replace(',', '.') - val.MARKETSHARE.toString().replace(',', '.')) >= 0) {
                    var warnamsHarian = "class='label label-kuning'";
                } else if ((Math.round(val.RKAP_MS.toString().replace(',', '.') - val.MARKETSHARE.toString().replace(',', '.')) < 0)) {
...
//                        "<td align='right'><span " + warnaRealrev + ">" + formatAngka(Math.round(val.REAL_REVENUE/1000000)) +"</span></td>" +
...
//                "<td class='success' align='right'><span " + warnaRealrev + "><b>" + formatAngka(Math.round(totalRealRevenue/1000000)) + "</b></span></td>" +//
...
//                "<td class='success'>&nbsp;</td>" +
    ...
    function get_pencapaian(tahun, bulan, hari) {
        var url = base_url + 'sg/PencapaianProvinsi/scodata/' + tahun + '/' + bulan + '/' + hari;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
    ...
    function get_pencapaian_bag(tahun, bulan, hari) {
        var url = base_url + 'sg/PencapaianProvinsi/scodataBag/' + tahun + '/' + bulan + '/' + hari;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
    ...
    function get_pencapaian_bulk(tahun, bulan, hari) {
        var url = base_url + 'sg/PencapaianProvinsi/scodataBulk/' + tahun + '/' + bulan + '/' + hari;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
    ...
    function chart(tahun, bulan, hari) {
        ...
        var url = base_url + 'sg/PencapaianProvinsi/getDataChart/' + tahun + '/' + bulan + '/' + hari;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
                        ...
                        callbacks: {
                            title: function (tooltipItem, data) {
                                ...
                                if (label == 'Wil 1') {
                                    ...
                                    wilayah = 'DKI, Jabar, Banten, Jateng, DIY';
                                } else if (label == 'Wil 3') {
                                    wilayah = 'Luar Pulau';
                                } else {
                                    wilayah = 'Curah';
    ...
    function chart1(tahun, bulan, hari) {
        var ctx2 = document.getElementById('chart1').getContext("2d");
        var url = base_url + 'sg/PencapaianProvinsi/revenueKabiro/' + tahun + '/' + bulan + '/' + hari;
        $.ajax({
            url: url,
            type: 'post',
            dataType: 'json',
