...
class Master_Menu extends CI_Controller {
	...
	private function make_isi_table($data)
	{
		...
		$no =1;
		foreach($data as $d){
			$btn_edit = '<button class="btn btn-info waves-effect " id="edit_menu" idpd="'.$d['ID_MENU'].'"><span class="fa fa-pencil-square-o"></span></button>';
			$btn_hapus = '<button class="btn btn-danger waves-effect" id="Hapus_menu" idpd="'.$d['ID_MENU'].'"><span class="fa fa-trash-o "></span></button>';
			$isi .= '<tr>';
			$isi .= '<td>'.$no.'</td>';
			$isi .= '<td>'.$d['ID_MENU'].'</td>';
			$isi .= '<td>'.$d['NAMA_MENU'].'</td>';
			$isi .= '<td>'.$d['ORDER_MENU'].'</td>';
	...
	public function Ajax_tambah_data()
	{
		...
		$this->Master_Menu_model->insert_data($nama_menu, $order_menu, $USER);
	...
	public function Ajax_simpan_edit()
	{
		...
		$this->Master_Menu_model->update_data($id, $nama_menu,$order_menu,$USER);
		$hasil = $this->make_isi_table($this->Master_Menu_model->get_data());
		echo json_encode(array('notify' => 1, 'html' => $hasil));
	}
