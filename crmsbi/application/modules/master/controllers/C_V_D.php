...
class C_V_D	 extends CI_Controller {

    function __construct(){
        parent::__construct();
		$this->load->model('C_V_Dmodel');
    ...
    public function index(){
		...
		$data['list'] = $this->make_isi_table($this->C_V_Dmodel->get_data());
		$data['list_jenis'] = $this->C_V_Dmodel->get_jenis();
		$data['list_menu'] = $this->C_V_Dmodel->get_menu();
		
		$this->template->display('C_V_Dview', $data);
		
    }
	
	// public function data_tabel(){
        
		// $data = $this->C_V_Dmodel->get_data();
	...
	public function Ajax_data_id()
	{
		...
		$hasil			= $this->C_V_Dmodel->get_data_id($id_user);
	...
	public function Ajax_tambah_data()
	{
		...
		$this->C_V_Dmodel->insert_data($id_jenis, $id_menu,$USER);
		$hasil = $this->make_isi_table($this->C_V_Dmodel->get_data());
    	echo json_encode(array('notify' => 1, 'html' => $hasil));
	}
	...
	{	
		...
		$this->C_V_Dmodel->update_data($id_user_akses, $id_jenis, $id_menu, $USER);
		$hasil = $this->make_isi_table($this->C_V_Dmodel->get_data());
		echo json_encode(array('notify' => 1, 'html' => $hasil));
	}
	...
	public function Ajax_hapus_data()
	{
		...
		$this->C_V_Dmodel->hapus_data($id_user, $USER);
		$hasil = $this->make_isi_table($this->C_V_Dmodel->get_data());
		echo json_encode(array('notify' => 1, 'html' => $hasil));
	}
}
