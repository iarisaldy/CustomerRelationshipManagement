...
class SAPConnection extends SAP {
     ...
     * @return SAPRFC_OK on success, SAPRFC_ERROR on failure
     */
    //edit by cyber_dy 19/06/2013
    //  function Connect($connect_data) {
    function Connect() {
        $file = "/opt/lampp/htdocs/scm/application/libraries/sap_mso_prod.php";
        if(file_exists($file)){
            $connect_data = "/opt/lampp/htdocs/scm/application/libraries/sap_mso_prod.php";
        }else{
             $connect_data = "/opt/lampp/htdocs/prod/scm/application/libraries/sap_mso_prod.php";
        }
       //$connect_data = "/opt/lampp/htdocs/sgg/include/connect/sap_mso_prod.php";
       //$connect_data = dirname(__FILE__)."/sap_mso_prod.php";
       //echo $connect_data;exit;
        if (file_exists($connect_data)) {   // edited by cyber_dy 20/06/2013
            if ($this->rfc)
                $this->Close();
...
//            print_r($this->userLogonData);
//            exit;
                $this->SetStatus(SAPRFC_OK, "");
            }
            $this->SetCodePage(4110); #amin
            return $this->GetStatus();
        }
        else { // edited by cyber_dy 20/06/2013
            $this->userLogonData = false;
            return $this->SetStatus(SAPRFC_ERROR, "pathnya tidak benar coba cek lagi " .
                            $connect_data);
        }
    }
    ...
    function Connect_si($connect_data) {
            if ($this->rfc)
                $this->Close();
            if (is_array($connect_data)) {
                $this->connType = SAPRFC_CONN_USERDEFINED;
    ...
    function Accept($progid = "", $gwhost = "", $gwserv = "") {
        ...
        if (empty($progid) && empty($gwhost) && empty($gwserv)) {
            for ($i = 0; $i < count($argv); $i++) {
                switch ($argv[$i]) {
                    case '-a' : $progid = $argv[$i + 1];
                        break;
                    case '-g' : $gwhost = $argv[$i + 1];
                        break;
                    case '-x' : $gwserv = $argv[$i + 1];
                        break;
                }
            }
