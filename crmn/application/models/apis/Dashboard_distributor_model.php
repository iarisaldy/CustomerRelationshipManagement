<?php
    ...
    class Dashboard_distributor_model extends CI_Model {
		...
		public function Add_schedule_kunjungan_mini($id_user, $data){
			...
			foreach($data as $d){
				...
				$ORDER_SEMEN =null;
				if(isset($d['ORDER_SEMEN'])){
					$ORDER_SEMEN		= $d['ORDER_SEMEN'];
				}
				
				$FLAG_UNPLANNED 	= $d['FLAG_UNPLANNED'];
				$FLAG_ORDER			= $d['FLAG_ORDER'];
				...
				if(count($hasil)==0){
					$sql2 = "
						...
						(ID_USER, ID_TOKO, TGL_RENCANA_KUNJUNGAN, CREATED_BY, CREATED_AT, CHECKIN_TIME, CHECKIN_LATITUDE, CHECKIN_LONGITUDE, CHECKOUT_TIME, CHECKOUT_LATITUDE, CHECKOUT_LONGITUDE, KETERANGAN, DELETED_MARK, ORDER_SEMEN, FLAG_UNPLANNED, FLAG_ORDER)
						VALUES 
						('$id_user', '$id_customer', '$TGL_KUN', '$id_user', SYSDATE, '$CHECKIN_TIME', '$CHECKIN_LATITUDE', '$CHECKIN_LONGITUDE', '$CHECKOUT_TIME', '$CHECKOUT_LATITUDE', '$CHECKOUT_LONGITUDE', '$keterangan', 0, '$ORDER_SEMEN', '$FLAG_UNPLANNED', '$FLAG_ORDER')
					";
		...
		public function Add_schedule_kunjungan($id_user, $data){
			...
			foreach($data as $d){
				...
				if(count($hasil)==0){
					$sql2 = "
						INSERT INTO CRMNEW_KUNJUNGAN_CUSTOMER 
						(ID_USER, ID_TOKO, TGL_RENCANA_KUNJUNGAN, CREATED_BY, CREATED_AT, CHECKIN_TIME, CHECKIN_LATITUDE, CHECKIN_LONGITUDE, CHECKOUT_TIME, CHECKOUT_LATITUDE, CHECKOUT_LONGITUDE, KETERANGAN, DELETED_MARK, ORDER_SEMEN)
						VALUES 
						('$id_user', '$id_customer', '$TGL_KUN', '$id_user', SYSDATE, '$CHECKIN_TIME', '$CHECKIN_LATITUDE', '$CHECKIN_LONGITUDE', '$CHECKOUT_TIME', '$CHECKOUT_LATITUDE', '$CHECKOUT_LONGITUDE', '$keterangan', 0, $ORDER_SEMEN)
					";
					$cek = $this->db->query($sql2);
					if($cek){
						$status=1;
