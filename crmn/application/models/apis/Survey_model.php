<?php
    ...
    class Survey_model extends CI_Model {
		...
		public function update_kunjungan($id_user, $hasil_survey){
			...
			foreach($hasil_survey as $h){
				...
				$order_semen		= $h['ORDER_SEMEN'];
				$FLAG_UNPLANNED		= $h['FLAG_UNPLANNED'];
				$FLAG_TIDAK_ORDER	= $h['FLAG_TIDAK_ORDER'];
		...
		/*
		ID_SURVEY_PROJECT, ID_KUNJUNGAN_CUSTOMER, VOLUME_WEEKLY, PRIODE_PROJECT, CREATE_BY, CREATE_AT, UPDATE_AT, IS_DELETE
		*/
		
		
		public function Add_survey_project($id_user, $project){
			$data = array();
			...
			foreach($project as $p){
				...
				$sqlIn = "
					INSERT INTO CRMNEW_SURVEY_PROJECT_TOKO
					(ID_KUNJUNGAN_CUSTOMER, NAMA_PROJECT, VOLUME, START_PROJECT, END_PROJECT, CREATE_BY, CREATE_AT, IS_DELETE)
					VALUES
					('$id_kc', '$np', '$v', to_date('$sp', 'YYYY-MM-DD'), to_date('$ep', 'YYYY-MM-DD'), '$id_user', SYSDATE, 0)
				";
				$setIn = $this->db->query($sqlIn);
				
				if($setIn){
					$sqlGet = "
						SELECT ID_SURVEY_PROJECT, NAMA_PROJECT, VOLUME, TO_CHAR(START_PROJECT,'YYYY-MM-DD') AS START_PROJECT, TO_CHAR(END_PROJECT,'YYYY-MM-DD') AS END_PROJECT,  ID_KUNJUNGAN_CUSTOMER
						FROM CRMNEW_SURVEY_PROJECT_TOKO
						WHERE 
							ID_KUNJUNGAN_CUSTOMER = '$id_kc' AND 
							NAMA_PROJECT = '$np' AND
							VOLUME = '$v' AND
							TO_CHAR(START_PROJECT,'YYYY-MM-DD') = '$sp' AND
							TO_CHAR(END_PROJECT,'YYYY-MM-DD') = '$ep'
					";
					$data_get = $this->db->query($sqlGet)->row();
					
					$hasil = array(
						'ID_SURVEY_PROJECT'		=> $data_get->ID_SURVEY_PROJECT,
						'NAMA_PROJECT'			=> $data_get->NAMA_PROJECT,
						'VOLUME'		 		=> $data_get->VOLUME,
						'START_PROJECT' 		=> $data_get->START_PROJECT,
						'END_PROJECT' 			=> $data_get->END_PROJECT,
						'ID_KUNJUNGAN_CUSTOMER' => $data_get->ID_KUNJUNGAN_CUSTOMER
					);
					array_push($data, $hasil);
				}
		...
		public function Update_survey_project($id_user, $project){
			$data = array();
			...
			foreach($project as $p){
				...
				$sqlUp = "
					UPDATE CRMNEW_SURVEY_PROJECT_TOKO
					SET 
						NAMA_PROJECT	= '$np',
						VOLUME			= '$v',
						START_PROJECT	= to_date('$sp', 'YYYY-MM-DD'),
						END_PROJECT		= to_date('$ep', 'YYYY-MM-DD'),
						UPDATE_AT		= SYSDATE,
						ID_KUNJUNGAN_CUSTOMER = '$id_kc'
					WHERE CREATE_BY = '$id_user' AND ID_SURVEY_PROJECT = '$id_sp'
				";
		...
		public function Delete_survey_project($id_user, $id_projects){
			
			$sql = "
				UPDATE CRMNEW_SURVEY_PROJECT_TOKO
				SET 
					IS_DELETE = 1 
				WHERE CREATE_BY = '$id_user' AND ID_SURVEY_PROJECT IN ($id_projects)
			";
			$hasil = $this->db->query($sql);
		...
		public function Get_survey_project($id_user, $limit){
			$sqlGet = "
				SELECT ID_SURVEY_PROJECT, NAMA_PROJECT, VOLUME, TO_CHAR(START_PROJECT,'YYYY-MM-DD') AS START_PROJECT, TO_CHAR(END_PROJECT,'YYYY-MM-DD') AS END_PROJECT,  ID_KUNJUNGAN_CUSTOMER
				FROM CRMNEW_SURVEY_PROJECT_TOKO
				WHERE 
					IS_DELETE = 0 AND
					CREATE_BY = '$id_user' AND 
					ROWNUM <= '$limit'
		...
		// ==================================================================================
		public function AddSurveyProject($id_user, $project){
			$data = array();
			...
			foreach($project as $p){
				...
				$where_kategori = "";
				if($id_k != null){
					$where_kategori = " ID_KATEGORI = '$id_k' AND ";
				}
				
				$sqlIn = "
					INSERT INTO CRMNEW_SURVEY_PROJECT_TOKO
					(ID_KUNJUNGAN_CUSTOMER, ID_KATEGORI, NAMA_PROJECT, VOLUME, START_PROJECT, END_PROJECT, CREATE_BY, CREATE_AT, IS_DELETE)
					VALUES
					('$id_kc', $id_k, '$np', '$v', to_date('$sp', 'YYYY-MM-DD'), to_date('$ep', 'YYYY-MM-DD'), '$id_user', SYSDATE, 0)
				";
				$setIn = $this->db->query($sqlIn);
				
				if($setIn){
					$sqlGet = "
						SELECT ID_SURVEY_PROJECT, ID_KATEGORI, NAMA_PROJECT, VOLUME, TO_CHAR(START_PROJECT,'YYYY-MM-DD') AS START_PROJECT, TO_CHAR(END_PROJECT,'YYYY-MM-DD') AS END_PROJECT,  ID_KUNJUNGAN_CUSTOMER
						FROM CRMNEW_SURVEY_PROJECT_TOKO
						WHERE 
							ID_KUNJUNGAN_CUSTOMER = '$id_kc' AND 
							NAMA_PROJECT = '$np' AND
		...
		public function UpdateSurveyProject($id_user, $project){
			$data = array();
		...
		public function DeleteSurveyProject($id_user, $id_projects){
		...
		public function GetSurveyProject($id_user, $limit){
			$sqlGet = "
				SELECT ID_SURVEY_PROJECT, ID_KATEGORI, NAMA_PROJECT, VOLUME, TO_CHAR(START_PROJECT,'YYYY-MM-DD') AS START_PROJECT, TO_CHAR(END_PROJECT,'YYYY-MM-DD') AS END_PROJECT,  ID_KUNJUNGAN_CUSTOMER
				FROM CRMNEW_SURVEY_PROJECT_TOKO
				WHERE 
