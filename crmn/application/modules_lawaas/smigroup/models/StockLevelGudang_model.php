...
class StockLevelGudang_model extends CI_Model {
    ...
    function getRptReal2(){
        ...
        $tgl = date('d-m-Y');
        //$tgl = date("d-m-Y", strtotime("$tgl -1 days"));
		
        // echo $tgl;
		$tgl_sebelumnya = date('d-m-Y', strtotime("$tgl - month"));
		
       $data =  $this->db->query("
            SELECT
	RPT.KODE_DA,
	NVL(TRUK.JML_TRUK, 0) AS JML_TRUK,
	KWANTUM,
	NVL (SISASO, 0) AS SISA_SO,
	NVL(TRUK.KWANTUMX, 0) AS KWANTUMX,
...
FROM
	(
		...
		--AND ITEM_NO = '121-301-0110' OR ITEM_NO 
		TGL_SPJ BETWEEN TO_DATE ('$tgl_sebelumnya', 'dd-mm-yyyy') AND 
...
LEFT JOIN 
    (
        ...
        GROUP BY
            KODE_DA
    ) TRUK ON RPT.KODE_DA=TRUK.KODE_DA
...
LEFT JOIN (
	...
	GROUP BY
		SHIP_TO_CODE
) SO ON SO.SHIP_TO_CODE = RPT.KODE_DA AND SO.SHIP_TO_CODE=TRUK.KODE_DA
                ");
...
ON SDP.KODE_DA = RPT.KODE_DA");
    ...
    function getStokRilis($kode_gudang, $tanggal) {
        ...
        $data = $this->db2->query("SELECT TB1.*, TB2.NM_GDG, (TB2.KAPASITAS*40/1000) KAPASITAS FROM (SELECT KODE_SHIPTO, TO_CHAR(TGL_RILIS,'DD') TANGGAL, 
            SUM(QTY_STOK) QTY_STOK, SUM(QTY_RILIS) QTY_RILIS 
                FROM CRM_GUDANG_SERVICE
                -- WHERE TO_CHAR(TGL_RILIS,'YYYYMM') = '$tanggal'
                WHERE TO_CHAR(TGL_RILIS,'YYYYMM') = '201802'
                AND KODE_SHIPTO = '$kode_gudang'
                GROUP BY KODE_SHIPTO, TGL_RILIS)TB1
                LEFT JOIN
                CRM_GUDANG TB2
	...
	public function get_kemampuan_bongkar_harian($kd_gudang){
		$this->db2 = $this->load->database('scmproduction', TRUE);
		$sql ="
			SELECT 
			UNLOADING_RATE_TON
			FROM CRM_GUDANG 
			WHERE KD_GDG='$kd_gudang'
		";
		return $this->db2->query($sql)->result_array();
	}
	public function Get_kapasitas_from_scm($kd_gudang){
		...
		$sql ="
			SELECT 
			KAPASITAS
			FROM CRM_GUDANG WHERE DELETE_MARK=0
			AND KD_GDG='$kd_gudang'
		";
		
		$hasil = $this->db2->query($sql)->result_array();
		
		if(count($hasil)>0){
			return $hasil[0]['KAPASITAS'];
		}
		else {
			return 0;
		}
    ...
    function getStokElogsHarianDist($kode_gudang, $tanggal){
		...
		// $sql ="
			// SELECT
                                    ...
                                    // MAX (NM_CUSTOMER) NM_GUDANG,
                                    // SUM (QTY_STOK) QTY_RILIS,
                                    // MAX (NM_CUSTOMER) NM_GDG,
                                    // MAX (KAPASITAS) KAPASITAS
                                  // FROM
                                  ...
                                  // -- AND TO_CHAR (TGL_STOK, 'YYYYMM') = '201802'
                                  // GROUP BY
                                    // TGL_STOK
                                  // ORDER BY
                                    // TGL_STOK
		...
		$sql ="
			SELECT
				MAX (HSTR.KODE_SHIPTO) KD_SHIPTO,
				TO_CHAR (HSTR.TGL_STOK, 'DD') TANGGAL,
				SUM (HSTR.QTY_STOK) QTY_STOK,
				MAX (C.NM_CUSTOMER) NM_GUDANG,
				SUM (HSTR.QTY_STOK) QTY_RILIS,
				MAX (C.NM_CUSTOMER) NM_GDG,
				MAX (C.KAPASITAS) KAPASITAS,
				P.BRT_PRODUK
			FROM
			   TPL_CRM_GUDANG_SERVICE_HSTR HSTR
			LEFT JOIN M_CUSTOMER C ON HSTR.KODE_SHIPTO = C.KD_CUSTOMER
			LEFT JOIN TPL_M_PRODUK P ON HSTR.ITEM_NO=P.KD_PRODUK
			WHERE
			KODE_SHIPTO = '$kode_gudang'
            AND TO_CHAR (TGL_STOK, 'YYYYMM') = '$tanggal'
			-- AND TO_CHAR (TGL_STOK, 'YYYYMM') = '201802'
			GROUP BY
			TGL_STOK, P.BRT_PRODUK
			ORDER BY
			TGL_STOK
		";
