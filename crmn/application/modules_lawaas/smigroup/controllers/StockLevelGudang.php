...
class StockLevelGudang extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('APIForca_model');
    }
	
	public function getkemampuanBongkar($kd_gudang){
		
		$hasil = $this->StockLevelGudang_model->get_kemampuan_bongkar_harian($kd_gudang);
		echo $hasil;
	}
    function index() {
    ...
    function getPeta() {
        ...
        // echo '<pre>';
        // print_r($transit_hasil);
        // echo '</pre>';
        foreach ($transit_hasil as $key => $value) {
            $sisa = $this->StockLevelGudang_model->getSisaSO(trim($value['KODE_DA']));
            $transit_hasil[$key]['SISASO'] = $sisa['SISASO']/25;
            $transit_hasil[$key]['SISA_TO'] = $sisa['SISA_TO']/25;
        }
...
//        }
        ...
        for ($i = 0; $i < sizeof($aarSql); $i++) {
            ...
            $dtElogs = $this->StockLevelGudang_model->getDataStokElogs($aarSql[$i]['KD_GDG']);
            ...
            foreach ($transit_hasil as $data) {
                ...
                // echo '<br>Data '.$data['KODE_DA'].' - '.  $aarSql[$i]['KD_GDG'];
                if ($data['KODE_DA'] == $aarSql[$i]['KD_GDG']) {
                	//$sisa_so = 0;
                	// echo $data['KODE_DA'] .' - '.$aarSql[$i]['KD_GDG'];
    ...
    function getStokTransit($kd_gdg) {
        ...
        for ($i = 0; $i < sizeof($transit_hasil); $i++) {
            ...
            if (true) {
                ...
                // $hasil[$i]['KEMAMPUAN_BONGKAR'] = $transit_hasil[$i]['KEMAMPUAN_BONGKAR'] * 1;
...
//                }
		...
		//====================================
		$kemampuan_bongkar_harian = $this->StockLevelGudang_model->get_kemampuan_bongkar_harian($kd_gdg);
        ...
        if (!isset($hasil)) {
            $hasil = array();
			echo json_encode(array('kemampuan_bongkar'=>round($kemampuan_bongkar_harian['0']['UNLOADING_RATE_TON']), 'data'=> $hasil));
    ...
    function getChart($kode_gudang, $date, $flag) {
        ...
        if($flag == 0){
        	...
        	// echo '<br> 1 -'.$flag;
            $data = $this->StockLevelGudang_model->getStokElogsHarianDist($kode_gudang, $date);
			
			$nilai_kapasitas = $this->StockLevelGudang_model->Get_kapasitas_from_scm($kode_gudang);
			// print_r($data);
        }
   ...
   //      	$arr['STOK'] 			= 0;
			...
			// $arr['TIGAPULUH'] 		= 0;
			// $arr['TUJUHPULUH'] 		= 0;
   //       // array_push($arr['STOK'], 0);
   //       //    array_push($arr['RILIS'], 0);
   //       //    array_push($arr['SERATUS'], 0);
   //       //    array_push($arr['TIGAPULUH'], 0);
   //       //    array_push($arr['TUJUHPULUH'], 0);
   //      }
   //      else{
	        ...
	        $arr['NAMA_GUDANG'] = isset($data[0]['NM_GDG']) ?$data[0]['NM_GDG'] :'';
	        $arr['TANGGAL'] = $tanggal;
	        $arr['STOK'] = array();
	        $arr['RILIS'] = array();
	        $arr['TIGAPULUH'] = array();
	        ...
	        foreach ($tanggal as $tgl) {
	            ...
	            foreach ($data as $value) {
	                if ($tgl == $value['TANGGAL']) {
						if($value['QTY_RILIS']!=0){
							if($value['BRT_PRODUK']==50){
								$qty[$tgl]['STOK'] = round($value['QTY_STOK'] /20);
								$qty[$tgl]['RILIS'] = round($value['QTY_RILIS']/20);
							}
							else if($value['BRT_PRODUK']==40){
								$qty[$tgl]['STOK'] = round($value['QTY_STOK'] /25);
								$qty[$tgl]['RILIS'] = round($value['QTY_RILIS']/25);
							}
							else {
								$qty[$tgl]['STOK'] = round($value['QTY_STOK']);
								$qty[$tgl]['RILIS'] = round($value['QTY_RILIS']);
							}
						}
	                    
	                }
	                //$qty[$tgl]['KAPASITAS'] = $value['KAPASITAS'];
					$qty[$tgl]['KAPASITAS'] = round($nilai_kapasitas/25);
	            }
	        }
	...
	//        $kapasitas = $data[0]['KAPASITAS'];
	        foreach ($qty as $value) {
				...
				if(empty($value['KAPASITAS'])){
					$value['KAPASITAS'] = 0;
				}
				else {					
					if($value['KAPASITAS'] == "" || $value['KAPASITAS'] == NULL){
						$value['KAPASITAS'] = 0;
					}
				}
...
//        $kd_perusahaan = $this->input->post('KODE_PERUSAHAAN');
    ...
    public function get_all_data_stok_forca(){
        date_default_timezone_set("Asia/Jakarta");
        // $tgl        = "2018-03-05";//$this->input->post('tanggal');
		$tgl 		= date("Y-m-d");//$this->input->post('tanggal');
    ...
    public function get_all_data_jual_forca(){
		$tgl 		= "2018-03-07";//$this->input->post('tanggal');
		$token 	    = "26f82158-566a-4c08-b183-a0ba8b2e7543";
    ...
    public function get_all_data_gr_forca(){
		$tgl_start 		= "2018-03-01";//$this->input->post('tanggal_start');
        $tgl_end 		= "2018-03-07";//$this->input->post('tanggal_end');
		$token 	    = "26f82158-566a-4c08-b183-a0ba8b2e7543";
    ...
    private function get_data_stok_from_forca($tgl, $token){
        ...
        $data_kirim = array();
        $data_kirim['token'] = '26f82158-566a-4c08-b183-a0ba8b2e7543';
        $data_kirim['movementdate'] = $tgl;
        $fields = json_encode($data_kirim);
        ...
        $this->APIForca_model->InsertDataStokForca($result);
        echo '<pre>';
        print_r($result);
        echo '</pre>';
    ...
    private function get_data_jual_from_forca($tgl, $token){
        ...
        // return $result;
        $result = json_decode($result);
        $this->APIForca_model->InsertDataJualForca($result);
        
        echo '<pre>';
        // print_r($result->resultdata);
        print_r($result);
        echo '</pre>';
    }
    ...
    private function get_data_gr_from_forca($tgl_start, $tgl_end, $token){
        ...
        $result = json_decode($result);
        $this->APIForca_model->InsertDataGRForca($result);
