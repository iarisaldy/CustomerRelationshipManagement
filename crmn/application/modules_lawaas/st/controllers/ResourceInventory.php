...
class ResourceInventory extends CI_Controller {
    ...
    function __construct() {
        ...
        if (!$this->session->userdata('is_login')) {
            $this->session->set_userdata(array("link"=>"sg/ResourceInventory"));
            redirect('login');
        }
    ...
    function index(){
        ...
        $plant_group = empty($plant_group) ? '4401' : $plant_group;
        $period = $tahun.'-'.$bulan;
    ...
    if(in_array($kode_kelompok, $kode_gypsum)){
        ...
        unset($result['minmax']['datasets'][3]); // permintaan pak ilmanza 8 maret 2017
    ...
    public function toExcel(){
        ...
        $namafile = 'Data '.$nama.' plant '.$listPlant[$plant].' '.$startDate.' sd '.$endDate.'.xls';
     #   $kode_gypsum = array(1300,1400);
        $gypsum_detail = array();
