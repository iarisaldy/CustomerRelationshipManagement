...
class Harga_competitor_all extends CI_Controller {

	public function __construct()
	{
		parent::__construct(); 
		$this->load->model('M_harga_competitor_all'); 
	}
	//fungsi yang digunakan untuk pemanggilan halaman home
	public function index()
	{
		$data['title'] = "Dashboard Harga Competitor All SKU"; 
		
		// $week_array = $this->getStartAndEndDate(1,2020);
		// print_r($week_array);
		
		$this->template->display('harga_competitor_all', $data); 
	}
	...
	$kategori = $_POST['kategori'];
		...
		$getProdukGroup = $this->M_harga_competitor_all->getProdukGroup($listFilterByVal, $listFilterSetVal, $filterTahun, $filterBulan, $filterMinggu, $where_bawahan, $kemasan, $kategori); 
		// echo json_encode($getProdukGroup);
		$Datagraph = array();
		$GrafikBeli = array();
		$GrafikJual = array(); 
		$ValueBeli = array();
		...
		foreach($getProdukGroup as $v){
			$getHargaBeli = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'BELI', $listFilterByVal, $listFilterSetVal, $filterTahun, $filterBulan, $filterMinggu, $where_bawahan, $kemasan, $kategori);
			$getHargaJual = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'JUAL', $listFilterByVal, $listFilterSetVal, $filterTahun, $filterBulan, $filterMinggu, $where_bawahan, $kemasan, $kategori);
			...
			if($filterBulan=='ALL'){
				...
				for($j=1;$j<=$bulanss;$j++){
					...
					$getHargaBeli = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'BELI', $listFilterByVal, $listFilterSetVal, $filterTahun, $Bulan, $filterMinggu, $where_bawahan, $kemasan, $kategori);
					$getHargaJual = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'JUAL', $listFilterByVal, $listFilterSetVal, $filterTahun, $Bulan, $filterMinggu, $where_bawahan, $kemasan, $kategori);
					$beli = (count($getHargaBeli)> 0 ? $getHargaBeli[0]['HARGA_PEMBELIAN'] : '0');
					$jual = (count($getHargaJual)> 0 ? $getHargaJual[0]['HARGA_PENJUALAN'] : '0');
				...
				for($j=1;$j<=5;$j++){
					$getHargaBeli = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'BELI', $listFilterByVal, $listFilterSetVal, $filterTahun, $filterBulan, $j, $where_bawahan, $kemasan, $kategori);
					$getHargaJual = $this->M_harga_competitor_all->getHarga($v['GROUP_ID'], 'JUAL', $listFilterByVal, $listFilterSetVal, $filterTahun, $filterBulan, $j, $where_bawahan, $kemasan, $kategori);
					$beli = (count($getHargaBeli)> 0 ? $getHargaBeli[0]['HARGA_PEMBELIAN'] : '0');
					$jual = (count($getHargaJual)> 0 ? $getHargaJual[0]['HARGA_PENJUALAN'] : '0');
					array_push($ValueBeli, floatval($beli));
					array_push($ValueJual, floatval($jual));  
	...
	public function get_cabang()
	{
		if(isset($_POST["id_gelombang"]) && $_POST["id_gelombang"] != "") {
			$q = $this->M_harga_competitor_all->get_cabang(intval($_POST["id_gelombang"]));
			$data = array();
			foreach($q as $r) {
	...
	{ 
			...
			$getPosisi = $this->M_harga_competitor_all->get_Posisi($id_position);
			$labelPosisi = array();   
			$dataPosisi = array();   
	...
	public function List_region(){
		...
		$id_user = $this->session->userdata("user_id");
		$jenisUser = $this->session->userdata("id_jenis_user"); 
		$list = $this->M_harga_competitor_all->getRegion($id_user, $jenisUser);
		if($list){
	...
	public function List_provinsi(){
		...
		$list = $this->M_harga_competitor_all->getProvinsi();
		if($list){
			foreach ($list as $list_Key => $list_Val) {
				$data[]  = array(
	...
	public function List_area(){
		...
		$list = $this->M_harga_competitor_all->getArea();
		if($list){
			foreach ($list as $list_Key => $list_Val) {
				$data[]  = array(
					$list_Val->ID_AREA,
	...
	public function List_distrik(){
		...
		$list = $this->M_harga_competitor_all->getDistrik();
		if($list){
			foreach ($list as $list_Key => $list_Val) {
				$data[]  = array(
					$list_Val->ID_DISTRIK,
