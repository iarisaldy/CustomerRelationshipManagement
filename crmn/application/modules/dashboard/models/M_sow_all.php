...
class M_sow_all Extends CI_Model
{
	...
	{ 	
		...
		if( $where_bawahan != '' && $where_bawahan=='DISTRIBUTOR'){
			$q = $this->db->query("	 			
						...
						WHERE CCR.KAPASITAS_JUAL IS NOT NULL AND ID_DISTRIBUTOR IN ( 
									SELECT 
			...
			$q = $this->db->query("  
					...
					WHERE CCR.KAPASITAS_JUAL IS NOT NULL AND NEW_REGION IN ( 
								SELECT 
										CUR.ID_REGION AS ID 
			...
			SELECT 
				...
				FROM MAPPING_TOKO_DIST_SALES MTDS
						...
						JOIN CRMNEW_CUSTOMER CC ON CC.ID_CUSTOMER = MTDS.ID_CUSTOMER
				WHERE  CC.KAPASITAS_JUAL IS NOT NULL AND
				{$where_bawahan}    
			...
			SELECT 
				...
				FROM MAPPING_TOKO_DIST_SALES MTDS
						...
						LEFT JOIN VIEW_DATA_TOKO_CUSTOMER VDTC
								ON VDTC.ID_CUSTOMER = MTDS.ID_CUSTOMER
						JOIN CRMNEW_CUSTOMER CC ON CC.ID_CUSTOMER = MTDS.ID_CUSTOMER
				WHERE  CC.KAPASITAS_JUAL IS NOT NULL 
				--AND
				--{$where_bawahan}    
				group by  MTDS.ID_CUSTOMER 
			 ");
