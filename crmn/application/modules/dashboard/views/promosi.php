            ...
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="card">
                    <div class="header bg-cyan">
                        <h2 style="padding-top: 0.2em;">Dashboard Promosi <span class="print_brand"></span></h2>
								...
								<div class="body" >
									<div class="row">
										<div class="col-md-12"> 
											...
											<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6" >
                                                <div class="form-group">
                                                    <div class="form-line" id="ListBrandSet"></div>
                                                </div>
                                            </div> 
									...
									<div class="col-sm-6">
										<h4 class="card-title">Hasil Survey <span class="print_brand"></span></h4>
										<figure class="highcharts-figure">
											 <div id="promosiChart"></div>
										</figure>
									</div> 
									<div class="col-sm-6">
										<h4 class="card-title">Promosi <span class="print_brand"></span></h4>
										<figure class="highcharts-figure">
...
$(document).ready(function() {  
	...
	ListFilterSet("0-ALL");ListBrandSet()
	// monitoring()
...
function monitoring(){
	...
	var nm_brand = brand != "" ? "Brand "+$("#brand option:selected").attr("nama") : "All Brand";
	$(".print_brand").html(nm_brand);
	...
	$("#keluhanChart, #daftarKeluhanChart, #promosiChart, #daftarPromosiChart").html('<tr><td colspan="20"><center><i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span><br><p style="margin-top: 1em;"> Loading data. Please wait ...</p></center></td></tr>');
			$.post("<?php echo base_url(); ?>dashboard/promosi/monitoring", {  
...
function ListBrandSet(){
	var response;
	var key = "#ListBrandSet";
	var type_list = 'Brand :';
	type_list += '<select id="brand" name="brand" class="form-control selectpicker show-tick" data-size="10" data-live-search="true">';
	 
		$.ajax({
            url: "<?php echo base_url(); ?>dashboard/keluhan/List_brand",
            type: 'GET',
            dataType: 'JSON',
            success: function(data){
				response = data['data'];
				type_list += '<option value="" selected> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; - ALL -</option>';
				for (var i = 0; i < response.length; i++) {
					type_list += '<option value="'+response[i][0]+'" nama="'+response[i][1]+'"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; '+response[i][1]+'</option>';
				}
				type_list += '</select>';
				$(key).html(type_list);
				$(".selectpicker").selectpicker("refresh");
