<?php
    class Model_AssignToko extends CI_Model {
		...
		public function listAssignSALES($idDistributor){
		
			$sql =" 
				SELECT
				TS.ID_ASSIGN_TOKO,
				TS.ID_USER,
				UC.NAMA AS NAMA_SALES,
				TS.ID_CUSTOMER,
				C.NAMA_TOKO,
				C.NAMA_DISTRIK AS KOTA,
				C.NAMA_KECAMATAN,
				C.ALAMAT
				FROM CRMNEW_ASSIGN_TOKO_SALES TS
				LEFT JOIN CRMNEW_USER UC ON TS.ID_USER=UC.ID_USER
				LEFT JOIN CRMNEW_BK_CUSTOMER C ON TS.ID_CUSTOMER=C.ID_CUSTOMER
				WHERE C.ID_DISTRIBUTOR='$idDistributor'
				AND TS.ID_USER IN (SELECT ID_USER FROM CRMNEW_USER_DISTRIBUTOR WHERE KODE_DISTRIBUTOR='$idDistributor' )
			";
			// ECHO $sql;
