...
class Customer extends CI_Controller {
    ...
    public function listCustomer(){
        ...
        $customer = $this->Customer_model->Get_data_list_customer($idDistributor, $idProvinsi, $kodeLt, $startDate, $endDate);
        // if(count($customer)==0){
        //     $customer = $this->Customer_model->Get_data_list_customer($idDistributor, $idProvinsi, $kodeLt, $startDate=null, $endDate);
        // }
        // print_r($customer);
        // print_r($_POST);
        // exit;
        ...
        for ($i=0; $i < count($customer); $i++) { 
            $data[] = array(
                ...
                $customer[$i]["KODE_CUSTOMER"],
                $customer[$i]["NAMA_TOKO"],
                $customer[$i]["NAMA_DISTRIBUTOR"],
                $customer[$i]["NAMA_PEMILIK"],
                $customer[$i]["NAMA_PROVINSI"],
                $customer[$i]["NAMA_DISTRIK"],
                $customer[$i]["NAMA_KECAMATAN"],
                //$customer[$i]["tipe_toko"],
                //$customer[$i]["nama_lt"],
                //$customer[$i]["CLUSTER_TOKO"],
                $customer[$i]["ALAMAT"],
                $customer[$i]["JML_KUNJUNGAN"],
                "<center><button id='btnUpdateTagging' data-idcustomer='".$customer[$i]["KODE_CUSTOMER"]."' class='btn btn-sm btn-warning'>Update Tagging</button></center>",
            );
            $j++;
        }
