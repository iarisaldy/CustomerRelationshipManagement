                            	...
                            	<div class="row">
                            		<div class="col-md-12">
										...
										<div class="col-lg-2 col-md-2 col-sm-3 col-xs-6">
											<div class="form-group">
												<div class="form-line">
													<b>SENIOR SALES MANAJER</b>
													<select id="pilihrsm" name="rsm" class="form-control show-tick">
														<option value="">All</option>
														<?php
														foreach ($list_rsm as $ListJenisValue) { ?>
														<option value="<?php echo $ListJenisValue->ID_RSM; ?>"><?php echo $ListJenisValue->NAMA_RSM; ?></option>
														<?php } ?>
													</select>
												</div>
											</div>
...
function tampil_data(){
	...
	var id_dis = $('#pilihdistributor option:selected').val();
		...
		$.ajax({
		url: '<?php echo site_url(); ?>laporan/Data_sales/ambildatagsm',
		type: 'POST',
		dataType : 'json',
		data: {
			"rsm" : rsm,
...
$(document).on("click", "#btnExport", function(e){
		e.preventDefault();
        var rsm = $('#pilihrsm').val();
        var id_dis = $('#pilihdistributor').val();

        window.open("<?php echo base_url()?>laporan/Data_sales/toExcel_GSM?rsm="+rsm+"&&id_dis="+id_dis,"_blank");
});
</script>