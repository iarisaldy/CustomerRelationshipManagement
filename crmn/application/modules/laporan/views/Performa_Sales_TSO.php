...
<script>
	...
	var tahun = $('#filterTahun option:selected').val();
		...
		$.ajax({
		url: '<?php echo site_url(); ?>laporan/Performa_Sales/GetData_TSO',
		type: 'POST',
		dataType : 'json',
		data: {
			"bulan" : bulan,
		...
		success: function(data){
			  ...
			  for(i=0; i<data.length; i++){
				...
				'<td><center><button type="button" id="detailSales" nama='+data[i].NAMA_SALES+' idpd='+data[i].ID_SALES+' class="btn btn-sm btn-info"><i class="fa fa-info"> Detail</i></button></center></td>'+
				'</tr>';
				no++;
				if(c == "x"){
					c = "y"  ;
