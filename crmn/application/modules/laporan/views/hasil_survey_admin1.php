...
$("#show_data").html('<tr><td colspan="12"><center><i class="fa fa-spinner fa-spin fa-3x fa-fw"></i><span class="sr-only">Loading...</span><br><p style="margin-top: 1em;"> Loading data. Please wait ...</p></center></td></tr>');
	...
	if(selesai>mulai){
		$.ajax({
			url: '<?php echo site_url(); ?>laporan/Hasil_survey/ambildataadmin',
			type: 'POST',
			dataType : 'json',
			data: {
				"id" : id,
...
function listDistributor(key){
	...
	$.ajax({
		url: "<?php echo base_url(); ?>laporan/Hasil_survey/DISTRIBUTOR_SURVEY/",
		type: 'POST',
		data: {
		...
		success: function(data){
			...
			$("#pilihdistributor").html(type_list);
			//alert(111);
			//$(".selectpicker").selectpicker("refresh");
		}
	});
...
$(document).on("click", "#btnExport", function(e){
    ...
    window.open("<?php echo base_url()?>laporan/Hasil_survey/toExcel_ADMIN?mulai="+mulai+"&selesai="+selesai+"&id="+id+"&region="+region, "_blank");
