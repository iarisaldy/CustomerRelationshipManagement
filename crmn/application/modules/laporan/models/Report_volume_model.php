...
class Report_volume_model extends CI_Model {
	...
	public function get_data_sidigi($star,$end, $id_user, $id_distributor=null, $idsales=null, $JENIS=null)
	{	
		...
		if($id_distributor!=null){
            $sql .= " AND KD_DISTRIBUTOR='$id_distributor' ";
        }
		if($idsales!=null){
			$customer_user = $this->Customer_user($idsales);
			$n=1;
			$fild3 = '';
			foreach($customer_user as $d){
				
				if(count($customer_user)>$n){
					$fild3 .= "'". $d['KD_CUSTOMER']."',";
				}
				else {
					$fild3 .= "'". $d['KD_CUSTOMER']."'";
