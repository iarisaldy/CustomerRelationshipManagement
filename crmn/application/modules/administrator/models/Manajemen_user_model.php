...
class Manajemen_user_model extends CI_Model {
    ...
    public function listArea($idProvinsi=null, $id_user=null){
        ...
        if($id_user!=null){
            $sql .= " WHERE ID_AREA NOT IN (SELECT ID_AREA FROM CRMNEW_USER_AREA WHERE ID_USER='$id_user') ";
        }
    ...
    public function listProvinsi($idRegion = null, $id_user=null){
        $sql ="
            ...
            WHERE NEW_REGION='$idRegion'
            
             
        ";
        
        if($id_user!=null){
            $sql .= " AND ID_PROVINSI NOT IN (SELECT ID_PROVINSI FROM CRMNEW_USER_PROVINSI WHERE ID_USER='$id_user') ";
        }
    ...
    public function listTSO($id_user=null){
        ...
        if($id_user!=null){
            $sql .= "   AND CU.ID_USER NOT IN (SELECT ID_TSO FROM CRMNEW_USER_TSO WHERE DELETE_MARK='0' AND ID_USER='$id_user') ";
        }
	...
	public function User_ASM($id_user=null){
		$sql ="
            ...
            UA.ID_ASM,
            U.NAMA
            FROM CRMNEW_USER_ASM UA
            LEFT JOIN CRMNEW_USER U ON UA.ID_ASM=U.ID_USER
            WHERE UA.DELETE_MARK='0'			
		";
    ...
    public function listRSM($id_user=null){
        ...
        if($id_user!=null){
            $sql .= " AND CU.ID_USER NOT IN (SELECT ID_RSM FROM CRMNEW_USER_RSM WHERE DELETE_MARK='0' AND ID_USER='$id_user') ";
        }
    ...
    public function addUserGUDANG($data){
        $addUser = $this->db->insert_batch('CRMNEW_USER_GUDANG', $data);
        if($this->db->affected_rows()){
            return true;
        } else {
            return false;
    ...
    public function updateUserGUDANG($idUserTSO, $data){
        $date = date('d-m-Y H:i:s');
        $this->db->set('UPDATE_DATE',"TO_DATE('$date','dd/mm/yyyy HH24:MI:SS')", false);
        $addUser = $this->db->where("NO_USER_GUDANG", $idUserTSO)->update('CRMNEW_USER_GUDANG', $data);
        if($this->db->affected_rows()){
            return true;
        } else {
            return false;
