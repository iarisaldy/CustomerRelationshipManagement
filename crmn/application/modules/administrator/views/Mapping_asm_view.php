...
<script>
    ...
    $(document).on("click", "#addUserTSO", function(e){
        ...
        var idUser = <?php echo $dt_user[0]['ID_USER'] ?>;
        $.ajax({
            url: "<?php echo base_url("administrator/Mapping_user_tso/addUserTSO_ASM/"); ?>",
            type: 'POST',
            dataType: 'JSON',
            data: {
                "id_user" : idUser,
    ...
    $(document).on("click", "#deleteUserTSO", function(e){
        ...
        var idUser = <?php echo $dt_user[0]['ID_USER'] ?>;
        $.ajax({
            url: "<?php echo base_url("administrator/Mapping_user_tso/deleteUserTSO_ASM"); ?>/"+iduserTSO,
            type: 'POST',
            dataType: 'JSON',
            data: {
    ...
    function listProvinsi(key, isi = null){
		$.ajax({
		url: '<?php echo site_url(); ?>administrator/Mapping_user_tso/Ajax_id_region_asm',
		type: 'POST',
		success: function(j){
			var dt = JSON.parse(j);
			var data =(dt.data[0]);
    ...
    function listTSO(key, idDistributor){
        var idUser = <?php echo $dt_user[0]['ID_USER'] ?>;
        $.ajax({
            url: "<?php echo base_url(); ?>administrator/Mapping_user_tso/TSO/",
            type: 'POST',
            data: {
                "id_user"    : idUser,
            },
            ...
            success: function(data){
                ...
                type_list += '<option value="00">Pilih TSO</option>';
                for (var i = 0; i < response.length; i++) {
    ...
    function userTSOList(idUser){
        $('#userTSOTable').dataTable({
            "destroy" : true,
            "ajax" : {
                url: "<?php echo base_url('administrator/Mapping_user_tso/userTSO'); ?>/"+idUser,
                type: "GET"
            },
        });
    }
