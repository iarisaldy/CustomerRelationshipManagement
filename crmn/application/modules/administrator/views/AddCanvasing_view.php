...
<script>
    ...
    $(document).on("change", "#listSales", function(){
        sales = $("#listSales").val();
        listCustomer("#selectCustomer", sales);
    ...
    $(document).on("click", "#addNewSchedule", function(e){
        ...
        $.ajax({
            url: "<?php echo base_url(); ?>administrator/Penugasan/actionAddCanvassing",
            type: "POST",
            data: {
    ...
    function listCustomer(key, sales=null){
        $.ajax({
		    url: "<?php echo base_url(); ?>administrator/Penugasan/Toko_sales/"+sales,
		    type: 'POST',
		    data: {
                "id_sales" : sales
		    },
		    ...
		    success: function(data){
                ...
                for (var i = 0; i < response.length; i++) {
                    type_list += '<option value="'+response[i]['KD_CUSTOMER']+'">'+response[i]['KD_CUSTOMER']+' - '+response[i]['NAMA_TOKO']+' - '+response[i]['ALAMAT']+'</option>';
                }
                type_list += '</select>';
