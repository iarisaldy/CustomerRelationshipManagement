							...
							<div class="body">
								<div class="container-fluid">
									...
									<ul class="nav nav-tabs tab-nav-right" role="tablist">
										...
										<li role="presentation"><a href="#TSO" data-toggle="tab">Sales</a></li>
									</ul>
...
<script>
    ...
    function listProvinsi(key, isi = null){
		$.ajax({
		url: '<?php echo site_url(); ?>administrator/Mapping_user_tso/Ajax_id_region',
		type: 'POST',
		success: function(j){
			var dt = JSON.parse(j);
			var data =(dt.data[0]);
    ...
    function listTSO(key, idDistributor){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                for (var i = 0; i < response.length; i++) {
                    type_list += '<option value="'+response[i]['ID_USER']+'">'+response[i]['USERNAME']+'</option>';
                }
                type_list += '</select>';
