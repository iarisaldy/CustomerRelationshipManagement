...
class Mapping extends CI_Controller {
	...
	private $filename = "TEMPLATE_UPLOADS_MAPPING_TOKO_SALES"; 	// menententukan nama file'
	...
	public function index(){
        ...
        $this->template->display('Upload_mapping_toko', $data);
	}
	...
	public function import(){
		...
		foreach($Importing_data as $dt_in){
			...
			$insert = $this->Mapping_model->mapping_toko_batch($id_sales, $id_toko, $id_dist);
			if($insert == 1){
				$data_sukses++;
			} else {
				$data_fail++;
		...
		foreach($sheet as $row){
			...
			if($numrow > 1){
				...
				$insert = $this->Mapping_model->mapping_toko_batch($row['A'], $row['C'],$row['E']);
				$data_sukses++;
			}
		...
		*/
		...
		redirect(site_url('customer/Mapping'));
	}
	...
	private function do_replacing_data($data_in){
		...
		$hasil 		= $this->Mapping_model->do_repalacing_data_mapping($data);
