...
require APPPATH . '/controllers/apis/Auth.php';
    ...
    class Document_crm extends Auth {
		...
		public function UpdateFoto_post(){
			...
			$status_uploads = null;
			for($i=0; $i<count($foto); $i++){
				$target_file = 'crmn/assets/Survey/';
				$target_file = $target_file. $nama[$i]; 
		...
		public function OldInsertfotoSatuan_post(){
		...
		public function InsertfotoSatuan_post(){
			$id_sales 		= $this->input->post("id_user");
			$id_kunjungan 	= $this->input->post("id_kunjungan_customer");
			$foto 			= $_FILES["foto"]['tmp_name'];
			$nama 			= $id_kunjungan.'_'.$_FILES["foto"]['name'];
			
			$ext = explode('.',$foto);
			//if($ext[1] != "jpg" or $ext[1] != "png" or $ext[1] != "JPG" or $ext[1] != "PNG"){
				//$response = array("status" => "gagal", "message" => "Extensi file tidak diperbolehkan.");
			//} else {
				...
				if($status_uploads != 0){
					$simpan = $this->Dokument_crm_model->Simpan_foto_survey($id_sales, $id_kunjungan, $path_file);
					$response = array("status" => "success", "message" => $pesan, "data" => $simpan);
				} else {
					$ambil = $this->Dokument_crm_model->cek_foto_survey_is_exist($id_sales, $id_kunjungan, $path_file);
					$response = array("status" => "success", "message" => $pesan, "data" => $ambil);
				}
